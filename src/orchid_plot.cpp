...
int _main(int argc, char** argv)
{
	...
	cxxopts::Options options("orchid_plot",
		"Multi-threaded pipelined Orchid k" + std::to_string(KMAX) + " plotter"
	...
	int k = 32;
	int port = 8444;			// 8444 = orchid, 9699 = chives
	int num_plots = 1;
	int num_threads = 4;
	int num_buckets = 256;
	int num_buckets_3 = 0;
	...
	if(contract_addr_str.empty() && pool_key_str.empty()) {
		std::cout << "Pool Public Key (for solo farming) or Pool Contract Address (for pool farming) needs to be specified via -p or -c, see `orchid_plot --help`." << std::endl;
		return -2;
	}
	if(!contract_addr_str.empty() && !pool_key_str.empty()) {
		std::cout << "Choose either Pool Public Key (for solo farming) or Pool Contract Address (for pool farming), see `orchid_plot --help`." << std::endl;
		return -2;
	}
	if(farmer_key_str.empty()) {
		std::cout << "Farmer Public Key (48 bytes) needs to be specified via -f, see `orchid keys show`." << std::endl;
		return -2;
	}
	if(tmp_dir.empty()) {
	...
	if(contract_addr_str.empty()) {
		...
		if(pool_key.size() != bls::G1Element::SIZE) {
			std::cout << "Invalid poolkey: " << bls::Util::HexStr(pool_key) << ", '" << pool_key_str
				<< "' (needs to be " << bls::G1Element::SIZE << " bytes, see `orchid keys show`)" << std::endl;
			return -2;
		}
	}
	else {
		try {
			...
			std::cout << "Invalid contract (address): 0x"
					...
					<< "' (" << ex.what() << ", see `orchid plotnft show`)" << std::endl;
			return -2;
		}
	}
	...
	{
		const std::string path = tmp_dir + ".orchid_plot_tmp";
	...
	{
		const std::string path = tmp_dir2 + ".orchid_plot_tmp2";
		if(auto file = FOPEN(path.c_str(), "wb")) {
			fclose(file);
			remove(path.c_str());
		} else {
	...
	{
		const std::string path = final_dir + ".orchid_plot_final";
		if(auto file = FOPEN(path.c_str(), "wb")) {
			fclose(file);
			remove(path.c_str());
		} else {
	...
	
	{
		...
		for(int i = 0; i < num_files_max; ++i) {
			const std::string path = tmp_dir + ".orchid_plot_tmp." + std::to_string(i);
			if(auto file = FOPEN(path.c_str(), "wb")) {
	...
	if(k < 32 && port == 8444) {
    	...
    	std::cout << "**                   !! k < 32 is not supported on orchid network !!                    **" << std::endl;
	...
	std::cout << "Multi-threaded pipelined Orchid k" + std::to_string(KMAX) + " plotter";
	#ifdef GIT_COMMIT_HASH
